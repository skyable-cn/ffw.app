...
public class PayController extends BaseController {
	...
	public Map<String, String> wxPay() {
		...
		data.put("notify_url", config.getNoticeUrl());// 支付成功后的回调地址
		...
		try {
			...
			if ("SUCCESS".equals(return_code) && return_code.equals(result_code)) {
				...
				String sign = WXPayUtil.generateSignature(response, config.getKey());
			...
			if ("goods".equals(type)) {
				...
				if (StringUtils.isNotEmpty(pd.getString("FROMWXOPEN_ID"))) {
					...
					if (StringUtils.isNotEmpty(pdgoods.getString("BACKMONEY0"))) {
						...
						String waitaccount = String
								.valueOf(DoubleUtil.sum(Double.parseDouble(pdmember0.getString("WAITACCOUNT")),
						...
						if (StringUtils.isNotEmpty(pdmember0.getString("FROMWXOPEN_ID"))) {
							if (StringUtils.isNotEmpty(pdgoods.getString("BACKMONEY1"))) {
								...
								String waitaccount1 = String
										.valueOf(Double.parseDouble(pdmember1.getString("WAITACCOUNT"))
								...
								if (StringUtils.isNotEmpty(pdmember1.getString("FROMWXOPEN_ID"))) {
									if (StringUtils.isNotEmpty(pdgoods.getString("BACKMONEY2"))) {
										...
										rest.post(IConstant.FFW_SERVICE_KEY, "member/edit", pdwaitaccount2,
												PageData.class);
									}
								}
							}
